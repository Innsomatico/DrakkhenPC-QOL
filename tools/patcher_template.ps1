...
# DRAKKHEN.COM mods: pipeline from the stock file, verified per-combination.
$stockComSha = '@@STOCK_COM_SHA@@'
$comOrder = '@@COM_ORDER@@' | ConvertFrom-Json
$comMods  = '@@COM_MODS@@'  | ConvertFrom-Json
$comShas  = '@@COM_SHAS@@'  | ConvertFrom-Json
$comDesc  = '@@COM_DESC@@'  | ConvertFrom-Json
$comSel = @($comOrder | Where-Object { $sel.Contains($_) })
if ($comSel.Count -gt 0) {
    $comp = Join-Path $GamePath 'DRAKKHEN.COM'
    $cbk = Join-Path $bak 'DRAKKHEN.COM'
    $srcPath = if (Test-Path $cbk) { $cbk } else { $comp }
    if ((Get-FileHash -Algorithm SHA256 $srcPath).Hash.ToLower() -ne $stockComSha) {
        Fail 'DRAKKHEN.COM (or its backup) is not the stock US build - not touching it.'
    }
    if (-not (Test-Path $cbk)) { Copy-Item $comp $cbk; Write-Host 'backed up DRAKKHEN.COM' }
    $cd = [IO.File]::ReadAllBytes($srcPath)
    foreach ($k in $comSel) {
        foreach ($r in $comMods.$k) {
    ...
    if ((Get-FileHash -Algorithm SHA256 $comp).Hash.ToLower() -ne $comShas.$key) {
        Copy-Item $cbk $comp -Force
        Fail 'patched DRAKKHEN.COM failed verification - original restored.'
    }
    Write-Host ('  DRAKKHEN.COM  - ' + (($comSel | ForEach-Object { $comDesc.$_ }) -join ', '))
}
